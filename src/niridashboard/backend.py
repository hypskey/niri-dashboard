        ...
        while not self.isInterruptionRequested():
            ...
            try:
                if command:
                    ...
                    self.finished_action.emit(True, "Window moved" if kind == "move" else "Window focused")
                ...
                if command:
                    self.finished_action.emit(False, str(error))
                    # Refresh after partially successful action sequences.
                    last = None
                else:
...
def demo_state():
    outputs = {name: {"model": model, "logical": {"x": i * 1920, "y": 0}} for i, (name, model) in enumerate([
